...
function SoulCareReportPDF({ funnelStats, ncStats, aiSummary, dateFrom, dateTo, chartImages, logoDataUri }) {
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        <View wrap={false}>
          <View style={pdfStyles.headerRow}>
            <View style={pdfStyles.headerLeft}>
              ...
              <View>
                ...
                <Text style={pdfStyles.subtitle}>RCCG The Envoys — Turning information into insight and insight into impact</Text>
              </View>
            </View>
          ...
          <Text style={pdfStyles.sectionLabel}>VIP → Membership Funnel</Text>
          <View style={pdfStyles.statRow}>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{funnelStats?.totalOverviews ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Overviews Submitted</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{funnelStats?.recommended ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Recommended (Yes)</Text>
              <Text style={pdfStyles.statSub}>{funnelStats?.recommendationRate ?? 0}% of overviews</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{funnelStats?.graduated ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Potential Envoys Graduated</Text>
              <Text style={pdfStyles.statSub}>{funnelStats?.graduationRate ?? 0}% graduation rate</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{funnelStats?.overallConversionRate ?? 0}%</Text>
              <Text style={pdfStyles.statLabel}>Overall VIP → Member</Text>
              <Text style={pdfStyles.statSub}>End-to-end conversion</Text>
            </View>
          </View>

          <Text style={pdfStyles.sectionLabel}>New Converts</Text>
          <View style={pdfStyles.statRow}>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{ncStats?.total ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Total New Converts</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{ncStats?.completed ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Fully Discipled</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{ncStats?.trainingDone ?? 0}</Text>
              <Text style={pdfStyles.statLabel}>Training Completed</Text>
            </View>
            <View style={pdfStyles.statBox}>
              <Text style={pdfStyles.statValue}>{ncStats?.retentionPct ?? 0}%</Text>
              <Text style={pdfStyles.statLabel}>Retention Rate</Text>
            </View>
          </View>

          {chartImages && (chartImages.vipDecision || chartImages.vipFunnel || chartImages.ncType || chartImages.ncMonth) && (
            <>
              <Text style={pdfStyles.sectionLabel}>Visual Breakdown</Text>
              <View style={pdfStyles.chartRow}>
                <View style={pdfStyles.chartBox}>
                  <Text style={pdfStyles.chartTitle}>VIP Decision Split</Text>
                  {chartImages.vipDecision && <Image src={chartImages.vipDecision} style={pdfStyles.chartImage} />}
                </View>
                <View style={pdfStyles.chartBox}>
                  <Text style={pdfStyles.chartTitle}>VIP Funnel Stages</Text>
                  {chartImages.vipFunnel && <Image src={chartImages.vipFunnel} style={pdfStyles.chartImage} />}
                </View>
                <View style={pdfStyles.chartBox}>
                  <Text style={pdfStyles.chartTitle}>New Converts by Type</Text>
                  {chartImages.ncType && <Image src={chartImages.ncType} style={pdfStyles.chartImage} />}
                </View>
                <View style={pdfStyles.chartBox}>
                  <Text style={pdfStyles.chartTitle}>Monthly Reach</Text>
                  {chartImages.ncMonth && <Image src={chartImages.ncMonth} style={pdfStyles.chartImage} />}
                </View>
              </View>
            </>
          )}
          ...
          {aiSummary && (
            <View style={pdfStyles.summaryBox}>
              <Text style={pdfStyles.summaryLabel}>Period Summary</Text>
              <Text style={pdfStyles.summaryText}>{aiSummary}</Text>
            </View>
          )}
        </View>
      </Page>
...
function BirthdaysWidget({ daysAhead = 7, showEmpty = true }) {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [daysAhead, currentYear]);

  const sendWish = (p, link) => {
          ...
          {people.map(p => {
            ...
            return (
              <div key={p.id} style={{
                ...
                {waLink && (
                  wished ? (
                    <span style={badge(C.green, C.greenLight, { fontSize: 12 })}>
                      <CheckCircle size={11} />Wished
                    </span>
                  ) : (
                    <button
                      onClick={() => sendWish(p, waLink)}
                      style={{ ...btn("gold", { padding: "6px 12px", fontSize: 12 }), border: "none", cursor: "pointer" }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// BirthdaysFromList — same visual card as BirthdaysWidget, but computes
// upcoming birthdays from an ALREADY-FETCHED list passed in by the parent,
// rather than fetching+merging first_timers/soul_care_contacts/church_members
// itself. Used where a page's birthday banner should reflect ONLY that
// page's own table records (Members Care, Stewards Care).
// ─────────────────────────────────────────────────────────────────────────────

function BirthdaysFromList({ people, daysAhead = 7, showEmpty = true }) {
  const [wishedSet, setWishedSet] = useState(new Set());
  const currentYear = new Date().getFullYear();

  useEffect(() => {
    let cancelled = false;
    fetchBirthdayWishesSet(currentYear).then(set => { if (!cancelled) setWishedSet(set); });
    return () => { cancelled = true; };
  }, [currentYear]);
  ...
  return (
    <div style={{ ...card, background: C.goldLight, border: `1px solid ${C.gold}30`, borderLeft: `3px solid ${C.gold}` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: upcoming.length ? 12 : 0 }}>
        <Gift size={14} color={C.goldDark} />
        <span style={{
          fontSize: 12, fontWeight: 700, fontFamily: F.head,
          textTransform: "uppercase", letterSpacing: ".07em", color: C.goldDark,
        ...
        {upcoming.length > 0 && (
          <span style={badge(C.goldDark, "#fff", { fontSize: 11 })}>{upcoming.length}</span>
        )}
        {upcoming.length === 0 && (
          <span style={{ fontSize: 12, color: C.textMuted, marginLeft: "auto" }}>
            None in the next {daysAhead} days
...
// ─────────────────────────────────────────────────────────────────────────────
// Soul Care Reporting Dashboard — free, deterministic narrative summary.
// Reads the same computed stats a paid AI call would have used, and writes
// a short multi-sentence summary via threshold-based phrasing. No network
// call, no cost, no dependency on any third party.
// ─────────────────────────────────────────────────────────────────────────────

function generateSoulCareSummary(funnelStats, ncStats) {
  if (!funnelStats) return "Not enough data yet to generate a summary for this period.";
  const sentences = [];

  if (funnelStats.totalOverviews === 0) {
    sentences.push("No VIP Retention Overviews were submitted in this period, so there's no funnel data to report on yet.");
  } else {
    sentences.push(
      `This period, ${funnelStats.totalOverviews} VIP Retention Overview${funnelStats.totalOverviews !== 1 ? "s were" : " was"} submitted, with ${funnelStats.recommended} (${funnelStats.recommendationRate}%) recommended for membership.`
    );
    ...
    if (funnelStats.recommendationRate >= 60) {
      sentences.push(`A ${funnelStats.recommendationRate}% recommendation rate is a strong sign the Experience Team's 3-week follow-up is genuinely connecting with first-timers.`);
    } else if (funnelStats.recommendationRate > 0 && funnelStats.recommendationRate < 40) {
...
function SoulCareSummaryPanel({ summaryText }) {
  return (
    <div style={{ ...card, background: C.goldLight, border: `1px solid ${C.gold}30` }}>
      ...
      <div style={{
        fontSize: 14, color: C.textPrimary, lineHeight: 1.75,
        background: "#fff", borderRadius: 8, padding: "14px 16px", border: `1px solid ${C.gold}20`,
      }}>
        {summaryText}
      </div>
    </div>
...
function SoulCareReportingDashboard() {
  ...
  const vipDecisionRef = useRef(null);
  const vipFunnelRef   = useRef(null);
  const ncTypeRef       = useRef(null);
  const ncMonthRef      = useRef(null);
  ...
  const summaryText = loading ? "" : generateSoulCareSummary(funnelStats, ncStats);

  const vipDecisionDonut = [
    { name: "Recommended",     value: funnelStats?.recommended || 0, color: C.green },
    { name: "Not Recommended", value: funnelStats?.declined || 0,    color: C.amber },
  ].filter(d => d.value > 0);

  const vipFunnelBars = funnelStats ? [
    { name: "Overviews Submitted", value: funnelStats.totalOverviews, color: C.blue },
    { name: "Recommended (Yes)",   value: funnelStats.recommended,    color: C.soul },
    { name: "Graduated to Member", value: funnelStats.graduated,      color: C.goldDark },
  ] : [];

  const ncByType = { "New Salvation": 0, "Rededication": 0 };
  ncData.forEach(r => { if (ncByType[r.conversion_type] !== undefined) ncByType[r.conversion_type]++; });
  const ncTypeDonut = Object.entries(ncByType)
    .map(([k, v]) => ({ name: k, value: v, color: k === "New Salvation" ? C.soul : C.gold }))
    .filter(d => d.value > 0);
  ...
  const downloadPdfWithCharts = async () => {
    ...
    try {
      ...
      const [vipDecision, vipFunnel, ncType, ncMonth, logoDataUri] = await Promise.all([
        captureRef(vipDecisionRef), captureRef(vipFunnelRef), captureRef(ncTypeRef), captureRef(ncMonthRef),
        fetchLogoDataUri(),
      ]);
      const doc = (
        <SoulCareReportPDF
          funnelStats={funnelStats} ncStats={ncStats} aiSummary={summaryText}
          dateFrom={dateFrom} dateTo={dateTo}
          chartImages={{ vipDecision, vipFunnel, ncType, ncMonth }}
      ...
      a.href = url; a.download = `soul_care_report_${new Date().toISOString().slice(0, 10)}.pdf`; a.click();
      ...
      <PageHeader
        ...
        action={
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {!loading && (
              <button style={btn("primary")} onClick={downloadPdfWithCharts} disabled={pdfGenerating}>
                <Download size={14} />{pdfGenerating ? "Preparing…" : "Download PDF"}
              </button>
            )}
          </div>
        }
        ...
        <>
          ...
          <div className="greport" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
            <div style={card}>
              ...
              <div ref={vipDecisionRef}>
                {vipDecisionDonut.length === 0
                  ? <PasEmpty label="No VIP overviews in this range" />
                  : <PasDonut data={vipDecisionDonut} centerValue={funnelStats?.totalOverviews ?? 0} centerLabel="Overviews Submitted" />}
              </div>
            </div>
            <div style={card}>
              <SH title="VIP Funnel Stages" icon={TrendingUp} />
              <div ref={vipFunnelRef}><PasOutcomeBars data={vipFunnelBars} /></div>
            </div>
            <div style={card}>
              <SH title="New Converts by Type" icon={Heart} />
              <div ref={ncTypeRef}>
                {ncTypeDonut.length === 0
                  ? <PasEmpty label="No New Converts in this range" />
                  : <PasDonut data={ncTypeDonut} centerValue={ncStats.total} centerLabel="New Converts" />}
              </div>
            </div>
            <div style={card}>
              <SH title="New Converts — Monthly Reach" icon={Activity} />
              <div ref={ncMonthRef}><PasOutcomeBars data={ncMonthReach} /></div>
            </div>
          </div>

          <div style={{ marginBottom: 20 }}>
            <SoulCareSummaryPanel summaryText={summaryText} />
          </div>
...
async function fetchBirthdayWishesSet(year) {
  const rows = await sb(`birthday_wishes?wish_year=eq.${year}&select=phone_key`).catch(() => []);
  return new Set((rows || []).map(r => r.phone_key));
}

async function logBirthdayWish(phone, year, wishedBy) {
...
function FlaggedRecords({ currentUser }) {
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  ...
  const load = useCallback(async () => {
      ...
      try {
        ...
        const resMap = {};
        (resolutions || []).forEach(r => { resMap[`${r.source_table}:${r.source_id}`] = r; });

        const cfRows  = (cf  || []).map(r => ({ ...r, _source: "call", _sourceTable: "call_feedback" }));
        const sclRows = (scl || []).map(r => ({ ...r, _source: "soulcare", _sourceTable: "soul_call_logs", person: peopleMap[String(r.person_id)] || {} }));

        const merged = [...cfRows, ...sclRows]
          .map(r => ({ ...r, resolution: resMap[`${r._sourceTable}:${r.id}`] || null }))
  ...
  const submitResolution = async (r) => {
    setSaving(true);
    try {
      await sb("pastoral_flag_resolutions", {
        method: "POST",
        body: JSON.stringify({
          source_table: r._sourceTable,
  ...
  const reopen = async (r) => {
    if (!window.confirm(`Reopen the case for ${(r._source === "soulcare" ? r.person?.full_name : r.first_timers?.full_name) || "this record"}?`)) return;
    try {
  ...
  const shownRows     = tab === "open" ? openRows : tab === "resolved" ? resolvedRows : rows;

  const agingCount = openRows.filter(r => daysOpen(r.created_at) >= 3).length;
  const { visibleCount, onScroll } = usePagedScroll(`flagged|${tab}`, shownRows.length, 10);

  return (
    <div className="page-enter">
      {CREDS_MISSING && <CredsBanner />}
      <PageHeader title="Flagged for Pastoral"
        subtitle={`${openRows.length} open record${openRows.length !== 1 ? "s" : ""} requiring pastoral attention`}
        action={agingCount > 0 && (
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
        {[
          { k: "open",     label: "Open",     count: openRows.length },
          { k: "resolved", label: "Resolved", count: resolvedRows.length },
          { k: "all",      label: "All",      count: rows.length },
        ].map(t => (
          <button key={t.k} onClick={() => setTab(t.k)} style={{
            padding: "6px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
            background: tab === t.k ? C.green : C.bg, color: tab === t.k ? "#fff" : C.textSecondary,
            border: `1.5px solid ${tab === t.k ? C.green : C.border}`,
          }}>{t.label} ({t.count})</button>
        ))}
      </div>
      ...
      {loading ? <SkeletonList rows={6} /> : (
        <div className="mc-scroll" onScroll={onScroll} style={{ maxHeight: 640, overflowY: "auto", paddingRight: 4 }}>
        <div style={{ display: "grid", gap: 10 }}>
          {shownRows.slice(0, visibleCount).map(r => {
            ...
            return (
              <div key={key} style={{
                ...card, padding: "14px 16px",
                borderLeft: `3px solid ${r.resolution ? C.green : aging ? C.danger : C.flag}`,
                opacity: r.resolution ? .85 : 1,
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "flex-start", flexWrap: "wrap" }}>
                    {r.resolution ? (
                      <span style={badge(C.green, C.greenLight)}><CheckCircle size={11} />Resolved</span>
                    ) : aging ? (
                ...
                {r.resolution ? (
                  <div style={{ marginTop: 10, background: C.greenLight, borderRadius: 8, padding: "10px 14px" }}>
                    <div style={{ fontSize: 12, color: C.greenDark, marginBottom: r.resolution.resolution_notes ? 4 : 0 }}>
                      <strong>Resolved</strong> by {r.resolution.resolved_by || "—"} on {(r.resolution.created_at || "").slice(0, 10)}
                    </div>
                    {r.resolution.resolution_notes && (
                      <div style={{ fontSize: 13, color: C.textSecondary, lineHeight: 1.5 }}>{r.resolution.resolution_notes}</div>
                    )}
                    <button style={btn("ghost", { padding: "5px 12px", fontSize: 11, marginTop: 8 })} onClick={() => reopen(r)}>
                      Reopen Case
                    </button>
                  </div>
                ) : isResolving ? (
                  <div style={{ marginTop: 10 }}>
                    <FieldInput label="Resolution notes (optional)" id={`resolve-${key}`} type="textarea"
                      value={resolutionNotes} onChange={e => setResolutionNotes(e.target.value)}
                      placeholder="What was done to resolve this?" />
                    <div style={{ display: "flex", gap: 8 }}>
                      <button style={btn("primary", { padding: "7px 16px", fontSize: 12 })} onClick={() => submitResolution(r)} disabled={saving}>
                        <CheckCircle size={13} />{saving ? "Saving…" : "Confirm Resolved"}
                      </button>
                      <button style={btn("ghost", { padding: "7px 16px", fontSize: 12 })} onClick={() => { setResolvingKey(null); setResolutionNotes(""); }}>
                        Cancel
                      </button>
                    </div>
                  </div>
                ) : (
                  <button style={btn("outline", { padding: "6px 14px", fontSize: 12, marginTop: 10 })}
                    onClick={() => { setResolvingKey(key); setResolutionNotes(""); }}>
          ...
          {shownRows.length === 0 && (
            <div style={{ ...card, textAlign: "center", padding: "3rem", color: C.textMuted }}>
              ...
              <div style={{ fontWeight: 700, fontFamily: F.head }}>
                {tab === "resolved" ? "No resolved cases yet" : "No flagged records"}
              </div>
              <div style={{ fontSize: 13, marginTop: 4 }}>
                {tab === "resolved" ? "Cases you resolve will show up here." : "Nothing requires pastoral attention right now."}
              </div>
            </div>
          )}
        </div>
...
function MemberProfile({ member, currentUser, role, onBack }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const logs = await sb(`soul_call_logs?person_table=eq.${table}&person_id=eq.${member.id}&select=*&order=call_date.desc,created_at.desc`);
        if (!cancelled) setCallHistory(logs || []);
  ...
  return (
    <div className="page-enter">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <div style={{ fontWeight: 700, fontSize: 14, fontFamily: F.head }}>Call History</div>
        <span style={{ fontSize: 12, color: C.textMuted }}>
          {loadingHistory ? "Loading…" : `${callHistory.length} call${callHistory.length !== 1 ? "s" : ""} logged`}
        </span>
      </div>
      {loadingHistory ? <SkeletonList rows={2} /> : callHistory.length === 0 ? (
        <div style={{ ...card, textAlign: "center", padding: "2rem", color: C.textMuted }}>
          <Phone size={22} style={{ marginBottom: 6, opacity: .4 }} />
          <div style={{ fontSize: 13 }}>No calls logged yet for {member.full_name}.</div>
        </div>
      ) : (
        <div style={{ display: "grid", gap: 8 }}>
          {callHistory.map(h => {
              ...
              <div key={h.id} style={{ ...card, padding: "12px 16px", borderLeft: `3px solid ${sm.color}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: h.notes ? 6 : 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 12, color: C.textMuted }}>· by {h.called_by || "—"}</span>
...
function StewardsCare({ currentUser, role, onViewProfile }) {
  ...
  const SORT_GETTERS = {
    ...
    gender: m => m.gender,
    dob: m => m.dob,
    status: m => m.membership_status || "Active",
    department: m => m.department?.toLowerCase(),
    position: m => m.position?.toLowerCase(),
    lastCalled: m => lastCalledByKey[m.id] || null,
  };
  ...
  const GRID = "minmax(186px,1.3fr) 175px minmax(150px,1fr) 62px 74px 96px 130px 110px 110px";
  const headCell = {
  ...
  return (
    <div className="page-enter">
      ...
      <PageHeader
        title="Stewards Care"
        subtitle="Stewards & Leadership Registry"
      ...
      <div style={{
        ...
        <Filter size={14} color={C.goldDark} style={{ flexShrink: 0 }} />
        ...
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          <div className="mc-scroll" style={{ overflow: "auto", maxHeight: 600, padding: "0 16px" }}>
            <div style={{
              display: "grid", gridTemplateColumns: GRID, gap: 10, alignItems: "center",
              padding: "10px 16px", background: C.bg, borderBottom: `1px solid ${C.border}`,
              position: "sticky", top: 0, zIndex: 3, minWidth: 1190,
            }}>
              <SortableHead label="Name" sortKey="name" activeKey={sortKey} dir={sortDir} onSort={onSort} style={{ ...headCell, ...stickyLeft(C.bg, 4) }} />
              {[
                ["Phone", "phone"], ["Email", "email"], ["Gender", "gender"], ["DOB", "dob"],
                ["Status", "status"], ["Department", "department"], ["Position", "position"], ["Last Called", "lastCalled"],
              ].map(([label, key]) => (
            ...
            {sorted.map((m, i) => {
              ...
              return (
                <div key={m.id} style={{
                  display: "grid", gridTemplateColumns: GRID, gap: 10, alignItems: "center",
                  padding: "10px 0", background: C.surface, minWidth: 1190,
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ marginTop: 12, fontSize: 12, color: C.textMuted }}>
          Showing <strong>{filtered.length}</strong> of <strong>{stewards.length}</strong> Steward{stewards.length !== 1 ? "s" : ""}
        </div>
      )}
    </div>
  );
...
function MembersCare({ currentUser, role, onViewProfile }) {
  ...
  const activeCount  = members.filter(m => (m.membership_status || "Active") === "Active").length;
  ...
  const sorted = sortKey ? genericSort(filtered, SORT_GETTERS[sortKey], sortDir) : filtered;
  const { visibleCount, onScroll } = usePagedScroll(`${fStatus}|${fMarital}|${fLife}|${search}|${sortKey}|${sortDir}`, sorted.length, 15);
  ...
  return (
    <div className="page-enter">
      ...
      <PageHeader
        title="Members Care"
        subtitle="Members registry for consistent follow-up"
        action={
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            {isAdmin && (
      ...
      <div style={{ marginBottom: 20 }}><BirthdaysFromList people={members} /></div>

      <div className="g4" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
        <StatCard label="Total Members"           value={total}        icon={Users}    accent={C.soul}     />
        <StatCard label="Active Members"          value={activeCount}  icon={CheckCircle} accent={C.green} />
        ...
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          <div className="mc-scroll" onScroll={onScroll} style={{ overflow: "auto", maxHeight: 600, padding: "0 16px" }}>
            ...
            <div style={{
              ...
              {[
                ["Phone", "phone"], ["Email", "email"], ["Gender", "gender"], ["DOB", "dob"],
                ["Category", "category"], ["Status", "status"], ["Last Called", "lastCalled"],
              ].map(([label, key]) => (
                <SortableHead key={key} label={label} sortKey={key} activeKey={sortKey} dir={sortDir} onSort={onSort} style={headCell} />
              ))}
            </div>
    ...
    (async () => {
      ...
      try {
        ...
        (logs || []).forEach(l => {
          ...
          if (!lastCalledMap[k] || l.call_date >= lastCalledMap[k]) {
            lastCalledMap[k] = l.call_date;
            lastStatusMap[k] = l.call_status;
          }
        ...
        const merged = [
          ...
          return {
            ...
            lastStatus: lastStatusMap[k] || null,
...
// genericSort — shared column-sort used by Members Care / Stewards Care tables.
// `getter` pulls the comparable value for a row; null/undefined always sort last
// regardless of direction, since an empty cell isn't meaningfully "less than".
function genericSort(rows, getter, dir) {
...
// SortableHead — a headCell that toggles asc/desc on click and shows a small
// arrow when it's the active sort column.
function SortableHead({ label, sortKey, activeKey, dir, onSort, style }) {
  const active = activeKey === sortKey;
  return (
    <div onClick={() => onSort(sortKey)} style={{
      ...style, cursor: "pointer", userSelect: "none",
      display: "flex", alignItems: "center", gap: 3,
      color: active ? C.textSecondary : style.color,
    }}>
      {label}
      <span style={{ fontSize: 9, opacity: active ? 1 : .25 }}>{active && dir === "desc" ? "▼" : "▲"}</span>
    </div>
  );
}
...
function LogSoulCallForm({ person, loggedBy, onCancel, onDone }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingHistory(true);
      try {
        const logs = await sb(`soul_call_logs?person_table=eq.${person._table}&person_id=eq.${person.id}&select=*&order=call_date.desc,created_at.desc&limit=5`);
    ...
    <div className="page-enter">
      ...
      <div style={{ maxWidth: 560, marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.textSecondary, fontFamily: F.head }}>Before you call — recent history</div>
          {history.length >= 5 && <span style={{ fontSize: 11, color: C.textMuted }}>showing last 5</span>}
        </div>
        {loadingHistory ? <SkeletonList rows={2} /> : history.length === 0 ? (
          <div style={{ ...card, padding: "14px 16px", color: C.textMuted, fontSize: 13 }}>
            No previous calls on record — this will be the first.
          </div>
        ) : (
          <div style={{ display: "grid", gap: 8 }}>
            {history.map(h => {
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div onClick={() => onViewProfile && onViewProfile(r)} style={{
                        fontWeight: 700, fontSize: 14, fontFamily: F.head, cursor: onViewProfile ? "pointer" : "default",
                        color: onViewProfile ? C.soul : C.textPrimary, textDecoration: onViewProfile ? "underline" : "none", textDecorationColor: `${C.soul}40`,
                      }}>{r.full_name}</div>
                      <div style={{ fontSize: 12, color: C.textMuted }}><PhoneLink phone={r.phone} withWhatsApp /></div>
...
function SoulCallQueue({ currentUser, currentUserRole = "soulcare", onLogCall, onViewProfile }) {
  const { data, loading, err, reload } = useSoulCallData();
  const [category, setCategory] = useState("Steward");
  const [search, setSearch]     = useState("");
  ...
  const byStatus = searched.filter(r => {
    if (statusFilter === "all") return true;
    if (statusFilter === "never") return !r.lastStatus;
    return r.lastStatus === statusFilter;
  });
  const ranked = sortByCallPriority(byStatus);
  const { visibleCount, onScroll } = usePagedScroll(`${category}|${search}|${statusFilter}|${isAdmin}`, ranked.length, 10);
  ...
  const statusTabs = [
    { k: "all",   label: "All",   count: searched.length },
    { k: "never", label: "Never Called", count: searched.filter(r => !r.lastStatus).length },
    ...Object.keys(SC_CALL_STATUS_META).map(s => ({
      k: s, label: s, count: searched.filter(r => r.lastStatus === s).length,
    })),
  ];
  ...
  return (
    <div className="page-enter">
      ...
      <div className="g4" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 24 }}>
        <StatCard label={`Total ${category === "Steward" ? "Stewards" : "Members"}`} value={searched.length} icon={Users}  accent={C.green} />
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
        {statusTabs.map(t => (
          <button key={t.k} onClick={() => setStatusFilter(t.k)} style={{
            padding: "6px 14px", borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: "pointer",
            background: statusFilter === t.k ? C.soul : C.bg, color: statusFilter === t.k ? "#fff" : C.textSecondary,
            border: `1.5px solid ${statusFilter === t.k ? C.soul : C.border}`,
          }}>{t.label} ({t.count})</button>
        ))}
      </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        <span onClick={() => onViewProfile && onViewProfile(r)} style={{
                          fontWeight: 700, fontSize: 14, fontFamily: F.head, cursor: onViewProfile ? "pointer" : "default",
                          color: onViewProfile ? C.soul : C.textPrimary, textDecoration: onViewProfile ? "underline" : "none", textDecorationColor: `${C.soul}40`,
                        }}>{r.full_name}</span>
    ...
    (async () => {
      ...
      try {
        const [sw, cm, asg, lg] = await Promise.all([
          sb("stewards?select=id,full_name,phone&limit=3000"),
          sb("church_members?select=id,full_name,phone&limit=3000"),
          sb("soul_call_assignments?select=*").catch(() => []),
...
function SoulCallsAnalyticsDashboard() {
  ...
  const downloadCSV = () => {
    ...
    const csvRows = [
      header.join(","),
      ...periodLogs.map(l => {
        const p = personMap[String(l.person_id)];
        return [
          escape(p?.full_name || "Unknown"), escape(p?.phone),
...
function NOMFirstTimersList({ currentUser }) {
  ...
  return (
    <div className="page-enter">
      {CREDS_MISSING && <CredsBanner />}
      <PageHeader title="Night of Mercy" subtitle="First-timers registered at Night of Mercy" />
...
function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const resolvedSet = new Set((resolutions || []).map(r => `${r.source_table}:${r.source_id}`));
        const openCf  = (cf  || []).filter(r => !resolvedSet.has(`call_feedback:${r.id}`)).length;
        const openScl = (scl || []).filter(r => !resolvedSet.has(`soul_call_logs:${r.id}`)).length;
        setFlagCount(openCf + openScl);
      } catch {}
    })();
  }, [session]);
  ...
  const renderContent = () => {
    ...
    if (active === "flagged") return <FlaggedRecords currentUser={user} />;
    ...
    if (active === "sc_assign_calls") {
      if (memberProfileTarget) {
        return <MemberProfile member={memberProfileTarget} currentUser={user} role={role} onBack={() => setMemberProfileTarget(null)} />;
      }
      return <AssignSoulCalls currentUser={user} onViewProfile={m => setMemberProfileTarget(m)} />;
    }
    ...
    if (active === "sc_call_queue" || active === "sc_my_calls") {
      ...
      return (
        <SoulCallQueue
          ...
          onViewProfile={m => setMemberProfileTarget(m)}
        />
      );
    }
